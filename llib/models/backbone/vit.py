...
class ViT(nn.Module):
    def __init__(self,
        ...
        if os.path.exists(pretrained_pth):
            self.map_state_dict_from_vitpose(pretrained_pth)
    ...
    def map_state_dict_from_vitpose(self, state_dict_pth=None):
        ...
        state_dict["backbone.pos_embed"] = self.interpolate_pos_encoding(img_w, img_h, original_pos_embed, patch_size)

        self.resize_patch_embed(state_dict)

        backbone_state_dict = {k: state_dict[k] for k in keys}
        state_dict = {k.replace("backbone.", ""): v for k, v in backbone_state_dict.items()}
        self.load_state_dict(state_dict, strict=True)

    def interpolate_pos_encoding(self, img_w, img_h, original_pos_embed, patch_size):
        curr_patch_h, curr_patch_w = 16, 12
        npatch = original_pos_embed.shape[1] - 1
        new_npatch = self.pos_embed.shape[1] - 1
        if npatch == new_npatch:# and img_w == img_h:
            print("No need to interpolate pos embedding!")
            return original_pos_embed
        print("interpolating pose embedding!", npatch, new_npatch)
    ...
    def resize_patch_embed(self, state_dict):
        ...
        if pretrained_conv_weight.shape[2] != self.patch_size:
            # Interpolate the weights to fit the new patch size
            print("interpolating weights from", pretrained_conv_weight.shape[2], "to ", self.patch_size)
            pretrained_conv_weight_resized = torch.nn.functional.interpolate(
                pretrained_conv_weight, size=(self.patch_size, self.patch_size), mode='bilinear', align_corners=False
            )
            ...
            print("No need to interpolate weights of patch embedding!")
