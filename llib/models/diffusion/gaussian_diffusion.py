...
# Part of this code is based on https://github.com/GuyTevet/motion-diffusion-model and https://github.com/sanweiliti/RoHM
...
class GaussianDiffusionQuestDiff:
    """
    ...
    def p_sample_with_grad(
        ...
        if grad_type == 'proj':
            grad_weight_2d_projection = 10.0  # 1e2
            grad_2d_projection = model.guide_2d_projection(batch, out, t, compute_grad='x_0')
    ...
    def p_sample_loop(
        ...
        if "uncertainty" in final:
            return torch.cat((final["sample"], final["uncertainty"]), dim=-1)
            # final["sample"] = torch.cat((final["sample"], final["uncertainty"]), dim=-1)
        if dump_steps is not None:
            return dump
    ...
    def training_step(self, model, batch, t, noise=None):
        if noise is None:
            noise = th.randn_like(batch['repr_clean'])
        batch['x_t'] = self.q_sample(batch['repr_clean'], t, noise=noise)
    ...
    def eval_step(self,
        ...
        Input:
            ...
            samler_step: if True, only sample for 100 steps, hardcoded
        """

        shape = list(batch['repr_clean'].shape)
        if timestep_respacing == '':
            model_output = self.p_sample_loop(model=model, batch=batch, shape=shape, progress=progress,
                                              clip_denoised=clip_denoised,
                                              cond_fn_with_grad=cond_fn_with_grad,
                                              grad_type=grad_type, 
                                              early_stop=early_stop, 
            ...
            model_output = self.ddim_sample_loop(model=model, batch=batch, shape=shape, progress=progress,
